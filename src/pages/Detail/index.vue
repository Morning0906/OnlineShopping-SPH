        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 加入购物车进行路由跳转之前，需要发请求，通知服务器进行相应存储 -->
  ...
  export default {
    ...
    methods: {
      ...
      async addShopcar(){
        ...
        // 2.当前这里派发了action，也向服务器请求了，需要判断加入购物车是否成功
        // 这行代码是调用仓库中的addUpdateShopCart，这个方法加上async，返回的一定是一个Promise【要么成功要么失败】
        ...
        try {
          ...
          skuNum:this.skuNum})
          // 3.服务器存储成功，进行路由跳转
        // 4.路由跳转时还需要将产品信息带给下一级的路由组件
        //下面这个方法路由跳转一级传参可以
        // this.$router.push({name:'addcartsuccess',query:{skuInfo:this.skuInfo,skuNum:this.skuNum}})
        
        // 一些简单是数据skuNum，通过query形式给路由组件传递过去
        // 产品信息的数据【比较复杂：skuInfo】，通过会话存储（不持久化，会话结束在消失）
        // 本地存储和会话存储一般存储的是字符串
